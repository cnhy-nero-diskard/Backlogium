...
- [x] 5.2 Run `./gradlew.bat :app:compileDebugKotlin --offline --no-daemon` and focused Library connected Compose tests when a device is available; debug Kotlin and Android-test compilation pass, and connected execution is unavailable because no device is attached.
