...
## 1. Persistence
- [ ] 1.1 Add `backfillMinutes: Int = 0` to the `Game` entity
- [ ] 1.2 Add a persisted "history imported" flag (e.g. `playtimeBackfilled: Boolean` on
  `PlayerProfile`, or a settings key)
- [ ] 1.3 Bump `BacklogiumDatabase` version; add an additive migration
...
- [ ] 1.4 `GameDao`: query/update to persist per-game `backfillMinutes` in a batch

## 2. Backfill action (domain)
- [ ] 2.1 A use-case that, when the flag is unset: for each game computes
...
- [ ] 2.2 Idempotence: if the flag is already set, the use-case is a no-op (no re-import, no
  double-count)

## 3. Feed the offset into XP
- [ ] 3.1 `GamificationUpdater.recompute`: build `GamePlaytimeInput.minutesPlayed =
  game.backfillMinutes + trackedMinutes(appId)` (join per-game backfill with tracked minutes)
- [ ] 3.2 Confirm untouched behavior when `backfillMinutes = 0` everywhere (pre-import installs
  compute exactly as today)
- [ ] 3.3 Update `GamificationUpdaterTest`: a game with `backfillMinutes` set yields the
...
## 4. UI (app-ui)
- [ ] 4.1 An "Import Steam history" control that invokes the backfill use-case, reflecting
  not-imported vs imported state from the flag
- [ ] 4.2 A brief confirmation before importing, noting it counts past playtime toward XP,
  is one-time, and that unmatched (no-HLTB) games count in full while matched games are capped
- [ ] 4.3 Reflect the resulting XP/level change on Home after import (existing observers
  already update; verify no stale state)

## 5. Validation
- [ ] 5.1 `:app` compiles; `:app:testDebugUnitTest` passes
