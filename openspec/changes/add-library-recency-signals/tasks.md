...
- [ ] 2.7 Ensure a poll never overwrites an existing `firstSeenAt`
- [ ] 2.8 Persist all three fields inside the poll's existing atomic commit — no separate write, so a return can never be stored without the playtime that justified it
- [ ] 2.9 Record the set of app ids a poll stamped as arrivals, and pass it to the acquisition announcement without a second query
...
- [ ] 3.3 Implement the derivation as a pure function of `(firstSeenAt, returnedToPlayAt, playtimeForever, firstSessionAt, now)`, returning at most one state — note it does **not** read `lastPlayedAt`, since dormancy was already decided at observation time
...
- [ ] 3.6 Derive `RETURNED` solely from `returnedToPlayAt` falling within the badge window — no gap is re-measured at read time
- [ ] 3.7 Add a bounded DAO query supplying the earliest session timestamp per game, so derivation does not become one query per row
...
- [ ] 8.11 Unit-test that `rtime_last_played` of `0` reads as unknown, and that an absent field parses without failing
