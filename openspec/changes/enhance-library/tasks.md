...
- [ ] 2.4 `HistoryScreen.kt:132`: "· 40m on goals" → focus wording, so the two screens agree
- [ ] 2.5 `SettingsScreen.kt:573`: the quest-mode chip label `QuestMode.GOAL_ONLY -> "Goal games only"`
...
- [ ] 2.6 Leave `isGoal`, `goalMinutesPlayed`, `observeGoalGames()`, `observeBacklog()`,
  `QuestMode.GOAL_ONLY`, and `Game.targetMinutes` untouched — user-facing copy only, no migration, no
  engine change
- [ ] 2.7 Grep for any remaining user-visible "goal" or "backlog" string and reconcile
- [ ] 2.8 Empty-state copy: check the "No games yet" message still reads correctly under the new labels

## 3. Per-list sorting
- [ ] 3.0 Widen `LibraryGame` + `Game.toDomain()` with `playtime2Weeks` — it exists on the `Game`
...
- [ ] 4.3 Section headings retained only for sections with matches (note `LibraryScreen.kt:121`
  currently renders the second heading unconditionally)
- [ ] 4.4 Empty state when the filter matches nothing, rendered **inside** the `LazyColumn` beneath
  the search field
- [ ] 4.5 Keep the pre-column early-return at `LibraryScreen.kt:78` keyed to the **unfiltered**
...
- [ ] 4.6 Clear action restores the full list
...
- [ ] 5.2 Expose per-game unlocked achievements **with their `snapshotPercent`** — note
...
- [ ] 5.3 `LibraryViewModel`: inject `SettingsRepository` and combine `ruleConfig` into the
...
- [ ] 5.4 Derive per game: `Gamification.gameXp(backfill + tracked, completionistMinutes, cfg)` +
  `Gamification.achievementXp(that game's unlocked achievements by snapshotPercent, cfg)`
- [ ] 5.5 Render as a compact badge beside the achievement badge, labelled as contributed XP
- [ ] 5.6 Correctness test: the badges sum to the profile's `totalXp`. Exactness is real, not
...
- [ ] 5.7 Scope that assertion to the games the Library shows. The engine sums achievements from
...
- [ ] 5.8 No changes to `:gamification` or `GamificationUpdater`
- [ ] 5.9 Re-check row density now that every row can also carry a progress bar; demote the XP badge
  if the row no longer reads cleanly

## 6. Batch progress + log
- [ ] 6.1 `HltbRepository.refreshBatch`: widen `onProgress` to
...
- [ ] 6.4a Handle two boundary edges: WorkManager clears `WorkInfo.progress` on completion, so empty
...
- [ ] 7.2a Gate the selection's refresh action on `refreshing`, the way `HltbControls` already
