...
- Keep an unobtrusive Cloud activity entry available in History whenever the reader is configured, including when there is no play or no cloud contribution in the loaded History window. Its detail names that window, explains an empty result or leads with recovered and cloud-timed sessions, then states last read and coverage limitations; retain the raw interval comparison in Diagnostics.
- Add opt-in-by-configuration routine cloud catch-up: Automatic is the initial policy, with a daily safety net and play-end opportunities; bounded 12-hour, daily, and 48-hour cadence overrides govern routine catch-up with a shared minimum gap. An Off / Manual only choice disables routine reads without disconnecting the reader. Keep manual Read now and existing accuracy-driven reads during Steam sync independent of this preference.
...
- `cloud-presence-reader`: add durable contribution provenance and bounded, disableable routine catch-up while preserving existing read, ingest, and placement authorities.
- `app-ui`: identify actual cloud contributions on History sessions and expose a focused Cloud activity detail and reader status for the loaded History window, even when no contributions are present.
- `app-settings`: expose Automatic, bounded cadence choices, and a manual-only choice, plus routine catch-up status and scope separately from manual reads and accuracy-driven sync reads.
