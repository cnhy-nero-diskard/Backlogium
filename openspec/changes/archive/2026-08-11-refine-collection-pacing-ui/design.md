...
The Library's right-aligned `header.jpg`, low opacity, and horizontal alpha mask remain a reusable Compose component for horizontal Library, collection-list, management, and game-detail cards. When `header.jpg` fails, the shared renderer tries `library_hero.jpg`, `capsule_616x353.jpg`, `hero_capsule.jpg`, and `library_600x900.jpg` in that order before leaving the themed surface unchanged. Library and collection overview grid tiles use Steam's portrait `hero_capsule.jpg` asset derived from the app id as their primary artwork, replacing the thumbnail-plus-faded-header composition. If the hero capsule fails, the grid renderer tries `library_hero.jpg`, `library_600x900.jpg`, `header.jpg`, and `capsule_616x353.jpg` in that order before showing the themed controller fallback. The Library full-detail destination and Collection detail overlay use the horizontal chain for their banner and sample the first decodable candidate for the surrounding accent wash. All chains preserve the existing card/tile geometry while Coil advances through CDN failures.
