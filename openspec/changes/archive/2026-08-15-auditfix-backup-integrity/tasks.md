...
- [x] 8.4 [P2] Close the equal-unlock-timestamp hole in order-independence: owner chose lower-`snapshotPercent`-wins as the tie-break. Spec gains the tie-break rule plus equal-timestamp and retired-field scenarios

## 9. PR #67 re-review follow-ups

- [x] 9.1 [P1] Apply the plausibility window to `achievement.unlockedAt` — it was still parse-only, so an impossible-but-valid 1970 timestamp passed preflight and then won `importedWins()` against a legitimate local unlock, permanently replacing a protected snapshot that ordinary syncs never repair. Also bounded `session.endAt`, which an in-range `startAt` did not constrain. Covered by `impossibleAchievementUnlockTimestamp_rejectedBeforeMerge` and `sessionEndingFarInTheFuture_rejectedEvenWhenStartIsValid`; the former verified to fail without the bound
- [x] 9.2 [P2] Make the import limit a memory bound rather than only a file-size bound: decoding now streams from a `BoundedInputStream` via `Json.decodeFromStream`, removing the `ByteArrayOutputStream`, its `toByteArray()` copy, and the UTF-16 `String` (each a multiple of the file size). Cap re-derived from an honest worst case — ~8.5 MB for a deliberately extreme account, ×4 allowance — and lowered from 256 MB to 32 MB
- [x] 9.3 [P3] Reword the equal-timestamp tie-break as a canonical, deterministic choice rather than evidence of observation order: rarity is a ratio and can fall as the player population grows, so "lower means earlier" was not sound. Rule is unchanged; comment and spec text corrected
