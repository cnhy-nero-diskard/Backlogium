...
The branch reached master and four real releases (v1.7.1–v1.7.4) were cut, unblocking 10.3–10.9,
all now verified on-device. The relaunch step surfaced a real bug (Android's
background-activity-launch restriction blocking the post-install relaunch), fixed and released
separately as `auditfix-update-relaunch-bal`. 10.7's corrupted-artifact scenario was reproduced by
temporarily swapping v1.7.4's published `.sha256` asset to a wrong value on GitHub (no root/proxy
available to tamper with the download itself), then restoring the correct value immediately after.

- [x] 10.3 Cut a test release through the real workflow and confirm both the APK and its `.sha256` are attached — v1.7.1–v1.7.4 all published with both assets; checksum verified against the downloaded APK
...
- [x] 10.6 Cancel at the system install prompt and confirm the app is unchanged and the artifact is gone — cancelling the real system confirmation dialog for v1.7.4 reported `INSTALL_FAILED_ABORTED: User rejected permissions`, versionName stayed at 1.7.3, and Update remained retryable; artifact deletion itself isn't independently observable without root, but the same `cleanup()` path is exercised and already unit-tested (9.7)
- [x] 10.7 Corrupt a downloaded artifact and confirm verification fails before the installer is reached — with v1.7.4's real `.sha256` asset swapped to a wrong value, the app downloaded the real APK, reported "The downloaded update failed its checksum check," left versionName at 1.7.3, and kept Update retryable
- [x] 10.8 Airplane mode: confirm checks fail silently and every other feature is unaffected — manual check reported "Check did not complete. Try again later." with no crash; Home, Library, History, and Analytics all continued rendering normally
