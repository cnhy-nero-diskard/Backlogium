...
- [x] 8.2 Validate `add-game-genres` with OpenSpec and confirm every scenario is represented by automated coverage or an explicit manual check. `openspec validate add-game-genres --strict` passes. Every scenario has automated coverage except two, both deferred to 8.3 for want of `androidx.work:work-testing`: `SteamSyncWorker`'s `runCatching` guard around `ensureEnqueued()`, and the worker's `Result.retry()`/continuation mapping. Compose layout (tile wrapping, non-clickability) is likewise device-verified; unit tests assert the state feeding it.
- [x] 8.3 On a device or emulator with a representative Steam library, verify background backfill/continuation, offline cached rendering, detail tile wrapping, genre text search, additive collection filtering, filter preservation after adding, and graceful behavior for unknown genres; record any unverified Store throttling behavior explicitly. Verified on-device by the author. Store throttling (HTTP 429) was not reproduced against the live Store and remains covered only by the fixture-driven transient-failure tests.
