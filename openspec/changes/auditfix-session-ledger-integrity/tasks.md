...
- [ ] 6.4 Confirm on a device that a Family Shared game observed by two overlapping presence checks yields one session in History, not two. **Partially verified, left unchecked**: on a physical device, live-tracked a real borrowed game (Zynga Poker, appId 3635220) through admission and ~11 app-foreground checks interleaved with periodic polls (`adb`-driven, database inspected directly via `run-as`/`exec-out`) — exactly one open session resulted, no duplicates anywhere in the library. Not a reproduction of the exact race, though: `onObservation()`'s vulnerable window is a local DB read-derive-write with no network call inside it, likely sub-millisecond to a few ms wide, and `adb`-driven triggers (each with tens-to-hundreds of ms of round-trip latency, plus Android's own ~700ms process-lifecycle debounce) can't land inside a window that narrow by chance. `WriteIntegrityDaoTest`'s coroutine-based tests remain the actual proof of the race itself; this device pass is corroborating evidence, not a substitute
- [x] 6.5 Sync the deltas into `openspec/specs/` via the archive workflow, not by hand — deferred to archive time
