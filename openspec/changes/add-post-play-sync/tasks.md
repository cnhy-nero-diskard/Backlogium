...
- [ ] 4.3 Use one unique work name per app id (`post-play-sync-<appId>`) with `REPLACE` for both the initial enqueue and each chained successor, so at most one request exists under a name and `REPLACE` can only ever supersede a schedule the user has replaced
...
- [ ] 6.3a Unit-test the attempt *timing*, asserting the enqueued delays are `1m, 2m, 5m` and therefore land at `T+0, T+1m, T+3m, T+8m` — the regression this guards is chaining the offsets as if they were delays
- [ ] 6.3b Unit-test that only one work request is ever pending under a given app id's name, so `REPLACE` cannot cancel a wanted sibling
- [ ] 6.3c Unit-test that a successful attempt enqueues no successor, so termination requires no cancellation
