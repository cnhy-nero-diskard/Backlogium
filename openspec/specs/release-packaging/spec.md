...
Defines what a published release of the app carries beyond the installable artifact itself: the
integrity digest a client uses to verify a download without trusting the transfer (see
`app-updates`), the audience-specific release notes derived from one deterministic model, and the
recognizable product/version naming applied across the release's Actions run, GitHub release, and
assets.
