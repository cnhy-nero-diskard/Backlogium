...
poll. A changed order SHALL be persisted only when the user completes a reorder gesture by releasing
the dragged card; a cancelled or abandoned drag SHALL leave the stored order unchanged and SHALL
leave the in-memory presentation consistent with the stored order, so a reorder the user saw started
but did not commit does not appear to land and then revert. Display order SHALL be independent of a
collection's mode, accent, creation time, and member order.
