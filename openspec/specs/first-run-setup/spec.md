...
Defines first-run setup as an ordered registry of named, independently opt-in stages: the extension
contract a further stage is registered through, per-stage ordering and declared defaults, the
boundary between work that runs while the setup surface is shown and work that detaches from it,
progress and notification semantics, failure isolation and retry, completion and skip semantics, and
re-running setup later from Settings.

Setup schedules work the app already performs and reports on it; it fetches, persists, and derives
nothing of its own. Every registered stage is optional, and setup completion is informational rather
than a gate on any part of the app.
