...
interface SteamReviewCacheDao {
     ...
     * promptly instead of interleaving them with refreshes of data it already has.
     *
    ...
    @Query(
        "SELECT games.appId FROM games " +
            ...
            "AND (steam_review_cache.appId IS NULL OR steam_review_cache.checkedAt < :staleBefore) " +
            "ORDER BY CASE WHEN steam_review_cache.appId IS NULL THEN 0 ELSE 1 END, " +
            "steam_review_cache.checkedAt ASC LIMIT :limit",
    )
