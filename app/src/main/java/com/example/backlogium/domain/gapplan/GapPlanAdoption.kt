...
/**
 * Turns an accepted variant into the draft the existing atomic collection save already
 * understands.
 *
 ...
 * forecast, and surviving sync like any other. A recommendation-specific table and an
 * auto-updating smart collection were both rejected — each would duplicate collection behaviour,
 * and automatic membership drift would undermine a plan the player committed to follow.
 */
object GapPlanAdoption {
     ...
     *
     * Membership is distinct and order-preserving: it is exactly what the player was looking at
     * when they confirmed, in the order they saw it.
     */
    fun toDraft(snapshot: GapPlanSnapshot, variant: GapPlanVariant): CollectionSaveDraft =
        CollectionSaveDraft(
            ...
            description = null,
            memberAppIds = variant.members.map { it.appId }.distinct(),
